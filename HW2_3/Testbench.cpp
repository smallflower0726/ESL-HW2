...
Testbench::Testbench(sc_module_name n)
    : sc_module(n), initiator("initiator"), output_rgb_raw_data_offset(54), counter_read(0), counter_write(0) {
  SC_THREAD(do_sobel);
}
...
void Testbench::do_sobel() {
  ...
  xBound = MASK_X / 2;            // 1
  yBound = MASK_Y / 2;            // 1
  for (y = -yBound; y != height + yBound - MASK_Y; ++y) {
	  for (x = -xBound; x != width + xBound; ++x) {
		  for (int k = 0; k < MASK_Y; ++k)
		  {
			  if (x >= 0 && x < width && (y + k) >= 0 && (y + k) < height) {
				  R = *(source_bitmap +
						  bytes_per_pixel * (width * (y + k) + (x)) + 2);
				  G = *(source_bitmap +
						  bytes_per_pixel * (width * (y + k) + (x)) + 1);
				  B = *(source_bitmap +
						  bytes_per_pixel * (width * (y + k) + (x)) + 0);
			  } else {
				  R = 0;
				  G = 0;
				  B = 0;
        ...
        counter_write += 3;
	//		  wait(1 * CLOCK_PERIOD, SC_NS); //emulate channel delay
		  }
	...
	//  	  wait(10 * CLOCK_PERIOD, SC_NS); //emulate channel delay
      }
